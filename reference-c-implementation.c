...
#define keyLen 128
serpent_encrypt(uint32_t _key[8],uint32_t _plaintext[4], 
		uint32_t ciphertext[4])
{
    uint32_t __attribute__((metamark("chiave"))) *key = _key;
    uint32_t __attribute__((metamark("plain"))) *plaintext = _plaintext;
